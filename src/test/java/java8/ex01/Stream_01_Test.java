...
public class Stream_01_Test {
	...
	public void test_stream_filter() throws Exception {
		...
		result = pizzas.stream().filter(p -> p.getPrice() >= 1300).collect(Collectors.toList());
	...
	public void test_stream_anyMatch() throws Exception {
		...
		result1 = pizzas.stream().anyMatch(p -> p.getPrice() >= 1300);

		// TODO valider si au moins une pizza à un prix >= 2000
		Boolean result2 = null;
		result2 = pizzas.stream().anyMatch(p -> p.getPrice() >= 2000);
	...
	public void test_stream_allMatch() throws Exception {
		...
		Boolean result1 = null;
		result1 = pizzas.stream().allMatch(p -> p.getPrice() >= 1300);

		// TODO valider que toutes les pizzas ont un prix >= 900
		Boolean result2 = null;
		result2 = pizzas.stream().allMatch(p -> p.getPrice() >= 900);
	...
	public void test_stream_noneMatch() throws Exception {
		...
		Boolean result1 = null;
		result1 = pizzas.stream().noneMatch(p -> p.getPrice() >= 2000);
	...
	public void test_stream_findFirst() throws Exception {
		...
		result = orders.stream().filter(o -> o.getCustomer().getFirstname().equals("Sophie")).findFirst();
	...
	public void test_stream_filter_adv() throws Exception {
		...
		result = orders.stream().filter(o -> o.getPizzas().stream().anyMatch(p -> p.getName().equals("Pépéroni"))).collect(Collectors.toList());
