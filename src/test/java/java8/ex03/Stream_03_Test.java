...
public class Stream_03_Test {
	...
	public void test_map_collect_joining() throws Exception {
		...
		// par le caractère "|"
		String result = null;
		result = customers.stream().map(Customer::getFirstname).sorted().collect(Collectors.joining("|"));
	...
	public void test_flatMap() throws Exception {
		...
		result = orders.stream().flatMap(o -> o.getPizzas().stream()).collect(toList());
	...
	public void test_flatMap_distinct() throws Exception {
		...
		List<Pizza> result = null;
		result = orders.stream().flatMap(o -> o.getPizzas().stream()).distinct().collect(toList());
	...
	public void test_grouping() throws Exception {
		...
		result = orders.stream().collect(groupingBy(Order::getCustomer));
	...
	public void test_partitionning() throws Exception {
		...
		result = pizzas.stream().collect(Collectors.partitioningBy(p -> p.getName().startsWith("L")));
